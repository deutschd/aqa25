...
# else:
#     scores = int(input("Введите число баллов :"))
#     if top <= 10 and scores >= 290:
#         print("Поздравляем, вы поступили!")
#         print("Бонусом вам будет начисляться стипендия.")
#     elif top <= 10 and scores < 290:
#         print("Поздравляем, вы поступили!")
...
#
# message = "Компьютер заблокирован. Вернёшь скейт - скажу код разблокировки!"
# password = 550
# print (message)

# `````````````````Дз```````````````````````#
# while True :
#     usr_int = int(input("Введите код: "))
#     if usr_int != password :
#         print (message)
#         if usr_int <= 1000 :
#             print ("Код не может начинатся с Нуля")
#     else :
#         print ("Код верный, завершаю работу...")

# `````````````````Дз / gpt```````````````````````#
# count = int(input("Сколько раз написать? :"))
# i = 0
#
# while i < count:
#     print(i,"."  ,"Я — программист!")
#     i += 1
